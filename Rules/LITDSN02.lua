...
local lightCharacteristics = 
{
	...
	[28] = {[1] = 'Al'},              -- Alternating
	[29] = {[1] = 'AlF F'},           -- fixed and alternating flashing
}
...
function LITDSN02(categoryOfLight, rhythmOfLight, lightColour, height, valueOfNominalRange, status)
	...
	if rhythmOfLight then
		...
		local lightCharacteristic = lightCharacteristics[rhythmOfLight.lightCharacteristic]
	...
	if rhythmOfLight then
		...
		if signalPeriod then
			description = string.format('%s%s%gs', description, space, signalPeriod)
			space = ''
		end
	end
	...
	-- Height
	if height then
		description = string.format('%s%s%gm', description, space, height)
		space = ''
	end

	-- Value of Nominal Range
	if valueOfNominalRange then
		description = string.format('%s%s%gM', description, space, valueOfNominalRange)
	end
